...
**你的任務：讀取規格書 → 定義型別 → 建立假資料 → 建立 service → 開發頁面 → 確保 `npm run build` 成功。**
...
9. 執行 `npm run build` 確認通過（若失敗自行修正）
...
Layout 已處理：RWD padding（手機 16px → 平板 24px → 桌面 32px）、maxWidth 1200px 居中、Sidebar 手機覆蓋式/桌面推擠式。
...
- 型別不可定義在 mock 檔案中
- 頁面最外層不可加額外 padding、margin、Container、maxWidth（Layout 已處理）
